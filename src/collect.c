/*
 * collect.c - Generate file list of relevant files by monitoring programs
 *
...
int main(int argc, char* argv[])
{
    ...
    int loglevel = *(int*)getConfig("LogLevel");//Config::get<int>("loglevel");
    int verbose  = *(int*)getConfig("Verbose");//Config::get<int>("verbose");
    ...
    if(0 == access((char*)getConfig("StartupLogFile"), F_OK))
        exclude_filenames.push_back((char*)getConfig("StartupLogFile"));
    ...
    while ((c = getopt_long (argc, argv, "hVvql:o:D:d:P:p:L:e:O:x:ku:", long_options, &option_index)) != EOF)
    {
        ...
        switch(c)
        {
            ...
            case 'e':
            {
                ...
                setConfig("EXT4Only", (bool*)value);//Config::set<bool>("ext4_only", value);
            }
                break;
            case 'O':
            {
                ...
                std::stringstream ss(optarg);
                ss >> std::boolalpha >> value;
                setConfig("ExcludeOpenFiles", (bool*)value);//Config::set<bool>("exclude_open_files", value);
            }
                break;
            case 'x':
                execute = optarg;
            ...
            case '?':
                if (optopt == 'o') // optional parameter for output is missing
                    ...
                    setConfig("EXT4Only", (bool*)true);//Config::set<bool>("ext4_only", true);
                else if(optopt == 'O')
                    setConfig("ExcludeOpenFiles", (bool*)true);//Config::set<bool>("exclude_open_files", true);
                else if(optopt == 'L')
    ...
    if(1 == getpid())
    {
        ...
        outPath = "/var/lib/e4rat/startup.log"; //Config::get<std::string>("startup_log_file").c_str();
        verbose = 0;
    }
    else
    {   //Config::get<bool>("exclude_open_files")
        if(true == *(bool*)getConfig("ExcludeOpenFiles") || exclude_filenames.size())
        {
            info("Generating exclude file list ...");
            try {
                if(true == *(bool*)getConfig("ExcludeOpenFiles"))
        ...
        if(outStream == stdout)
            ...
            outPath = "/var/lib/e4rat/startup2.log";//"./e4rat-collect.log";
    ...
    //Config::get<bool>("ext4_only")
    if( (bool*)getConfig("EXT4Only"))
    ...
    if(execute || 1 == getpid())
    {
        ...
        switch(fork())
        {
            ...
            default:
                ...
                {   //Config::get<std::string>("init").c_str()
                    notice("Execute `%s' ...", (char*)getConfig("Init"));
                    execv((char*)getConfig("Init"), argv);
                }
                sleep(1);
                exit(0);
         }
    ...
    if(create_pid_late)
    {
        ...
        unsigned int timeout = *(unsigned int*)getConfig("Timeout");//Config::get<unsigned int>("timeout");
        if(timeout)
        {
            sigaction(SIGALRM, &sa, NULL);
            alarm(*(unsigned int*)getConfig("Timeout"));
        ...
        else
        {
            if(pc == false)
                ...
                notice("Signal collector to stop by calling `%s -k'", PROGRAM_NAME);
        }
    }
    else
    ...
    if(outStream != stdout)
        ...
        //notice("Test output %s", Config::get<std::string>("startup_log_file").c_str());
...
err1:
    ...
    exit(1);
err2:
    if(getpid() == 1) //Config::get<std::string>("init").c_str()
         execv((char*)getConfig("Init"), argv);
    unlink(PID_FILE);
    exit(1);
}
